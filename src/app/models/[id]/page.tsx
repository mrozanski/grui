...
import { ArrowLeft, ExternalLink, MapPin, Calendar, Package, FileText, Guitar, DollarSign, Users, Building } from "lucide-react"
...
function formatCurrency(amount: number | null, currency: string | null = 'USD') {
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        {model.specifications.length > 0 && (
          <Card>
            ...
            <CardContent>
              <div className="space-y-2">
                {model.specifications.slice(0, 10).map((spec) => (
                  <div key={spec.id} className="flex justify-between items-center py-2 border-b last:border-0">
                    <span className="text-gray-600">{spec.specification_name}</span>
                    <span className="font-medium text-right">{spec.specification_value}</span>
                  </div>
                ))}
                {model.specifications.length > 10 && (
                  <p className="text-sm text-gray-500 pt-2">
                    And {model.specifications.length - 10} more specifications...
                  </p>
                )}
              </div>
            </CardContent>
              ...
              <div className="space-y-2">
                {model.finishes.slice(0, 10).map((finish) => (
                  <div key={finish.id} className="flex justify-between items-center py-2 border-b last:border-0">
                    ...
                    <span className="font-medium text-right">{finish.finish_code || 'N/A'}</span>
                  </div>
                ))}
